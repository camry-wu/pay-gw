...
package object controllers {
	...
	def str2DateTime(sdate: Option[String]) : Option[models.DateTime] = {
		sdate match {
			case Some(s) => Some(new models.DateTime(dateFormat.parse(s)))
			case _ => None
		}
	}

	def date2str(d: Option[models.DateTime]) : Option[String] = {
		val dateFormat: DateTimeFormatter = DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss")
		Some(dateFormat.print(d.getMillis))
	}
